...
const Drawer = ({ handleClose, open, children }) => {
    const drawerStyle = {
        transform: (open=='teacher' || open=='student' || open == 'filter' || open == 'view') ? "translateX(0)" : "translateX(100%)",
    };

    return (
        <>
            <div className={`drawer-overlay ${(open=='teacher' || open=='student' || open == 'filter' || open == 'view') ? 'opacity-100 z-50 block' : 'opacity-0 z-0 hidden'}`} onClick={handleClose}></div>
