...
const AddTeacher = ({ role, open, setIsEdit, isEdit, handleClose }) => {
    const fileRef = useRef(null);
    const dispatch = useDispatch();
    let user = useSelector(state => state.auth.user);
    let loading = useSelector((state) => state.role.loading.createRole);
    ...
    const buildPayload = (values) => {
        ...
        Object.keys(values).forEach((key) => {
            ...
            if (key === "dob" && values.dob) {
                formData.append(key, format(values.dob, "yyyy-MM-dd"));
            }
            else if (key === "file") {
                if (values.file) {
    ...
    return (
        <form onSubmit={formik.handleSubmit} className="h-calc(100% - 68px)">
            <div className="flex flex-wrap gap-4 items-start px-4 pb-6">
                ...
                <button type="submit" className="btn btn_with_text navy-btn" disabled={loading || !(formik.isValid && formik.dirty)}>
                    {loading
                        ? isEdit
                            ? <><Loader className="own-icon animate-spin [animation-duration:2s]" /> Updating </>
                            : <><Loader className="own-icon animate-spin [animation-duration:2s]" /> Creating </>
                        : isEdit
                            ? "Update "
                            : "Create "}
                    {role}
